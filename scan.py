...
def process_scan(time_window):
    ...
    payload = {"_id": int(time.time()), "time": now.strftime("%c"), "fingerprints": int(len(fingerprints2)), "signals": fingerprints2}
    write_to_db_full('127.0.0.1:27017', payload)
    
    new_payload = {"_id": now.strftime("%c"), "day": now.strftime("%A"), "hour": now.hour, "minute": now.minute, "customers": int(len(fingerprints2))}
...
def main():
    ...
    parser.add_argument("-s", "--sleep", default=600, help="set sleep time in seconds (default 600)" )
    ...
    while True:
		...
		write_to_db(args.database, payload)
                # r = requests.post(args.database + "/reversefingerprint", json=payload)
                # logger.debug("Sent to database with status code: " + str(r.status_code))
